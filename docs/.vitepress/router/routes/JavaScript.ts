/*
 * @file: JavaScript 路由
 * @author: DontK
 * @LastEditTime: 2024-04-15 11:15:41
 */
export default [
  {
    text: "ES6系列",
    collapsed: false,
    items: [
      {
        text: "函数的扩展",
        link: "/page/frontend/JavaScript/0_FunctionExtension",
      },
      {
        text: "对象的拓展",
        link: "/page/frontend/JavaScript/0_ObjectExtension",
      },
      {
        text: "运算符的拓展",
        link: "/page/frontend/JavaScript/0_OperrationExtension",
      },
    ],
  },
  {
    text: "深入系列",
    collapsed: false,
    items: [
      {
        text: "原型链",
        link: "/page/frontend/JavaScript/1_Prototype",
      },
      {
        text: "作用域",
        link: "/page/frontend/JavaScript/1_Scope",
      },
      {
        text: "执行上下文栈",
        link: "/page/frontend/JavaScript/1_ExecutionContextStack",
      },
      {
        text: "执行上下文",
        link: "/page/frontend/JavaScript/1_ExecutionContext",
      },
      {
        text: "作用域链",
        link: "/page/frontend/JavaScript/1_ScopeChain",
      },
      {
        text: "this",
        link: "/page/frontend/JavaScript/1_This",
      },
    ],
  },
  {
    text: "专题系列",
    collapsed: false,
    items: [
      {
        text: "闭包",
        link: "/page/frontend/JavaScript/2_Closure",
      },
      {
        text: "apply|call|bind",
        link: "/page/frontend/JavaScript/2_ThisTransfer",
      },
      {
        text: "创建对象的多种方式",
        link: "/page/frontend/JavaScript/2_CreateObject",
      },
      {
        text: "继承的多种方式",
        link: "/page/frontend/JavaScript/2_ObjectExtend",
      },
      {
        text: "防抖",
        link: "/page/frontend/JavaScript/2_Debounce",
      },
      {
        text: "节流",
        link: "/page/frontend/JavaScript/2_Throttle",
      },
      {
        text: "类型判断",
        link: "/page/frontend/JavaScript/2_TypeJudge",
      },
      {
        text: "Object.assign",
        link: "/page/frontend/JavaScript/2_ObjectAssign",
      },
      {
        text: "深拷贝和浅拷贝",
        link: "/page/frontend/JavaScript/2_Copy",
      },
      {
        text: "队列-事件循环",
        link: "/page/frontend/JavaScript/2_Queue",
      },
      {
        text: "集合-Set 和 字典-Map",
        link: "/page/frontend/JavaScript/2_SetAndMap",
      },
      {
        text: "树-深/广度优先遍历",
        link: "/page/frontend/JavaScript/2_Tree",
      },
      {
        text: "函数柯里化",
        link: "/page/frontend/JavaScript/2_Currying",
      },
      {
        text: "Object.defineProperty",
        link: "/page/frontend/JavaScript/2_ObjectDefineProperty",
      },
    ],
  },
  {
    text: "UtilsOthers系列",
    collapsed: false,
    items: [
      {
        text: "字符串处理方法",
        link: "/page/frontend/JavaScript/7_OperatorString",
      },
      {
        text: "浏览器操作dom操作",
        link: "/page/frontend/JavaScript/7_WindowOrDomOrHtmlCss",
      },
      {
        text: "设备判断ios.mobble.mac.weixin",
        link: "/page/frontend/JavaScript/7_DeviceJudgment",
      },
      {
        text: "运算符操作",
        link: "/page/frontend/JavaScript/7_OperatorOperations",
      },
      {
        text: "颜色RGB十六进制互转",
        link: "/page/frontend/JavaScript/7_RgbOrHex",
      },
      {
        text: "设置读取删除Cookie",
        link: "/page/frontend/JavaScript/7_SetCookie",
      },
      {
        text: "本地会话存储类操作",
        link: "/page/frontend/JavaScript/7_MyCache",
      },
      {
        text: "解析URL参数",
        link: "/page/frontend/JavaScript/7_SearchParams",
      },
      {
        text: "获取文件后缀名",
        link: "/page/frontend/JavaScript/7_GetExt",
      },
    ],
  },
  {
    text: "UtilsArray系列",
    collapsed: false,
    items: [
      {
        text: "数组常用方法",
        link: "/page/frontend/JavaScript/3_ArrayFunction",
      },
      {
        text: "数组reduce方法总结",
        link: "/page/frontend/JavaScript/3_ArrayReduce",
      },
      {
        text: "数组去重",
        link: "/page/frontend/JavaScript/3_ArrayDeduplication",
      },
      {
        text: "数组扁平化",
        link: "/page/frontend/JavaScript/3_ArrayFlatten",
      },
      {
        text: "数组最大值和最小值",
        link: "/page/frontend/JavaScript/3_ArrayMaxMin",
      },
      {
        text: "获取随机数",
        link: "/page/frontend/JavaScript/3_Random",
      },
      {
        text: "两个参数值是否相等",
        link: "/page/frontend/JavaScript/3_TwoEqual",
      },
      {
        text: "矩阵交换行和列",
        link: "/page/frontend/JavaScript/3_ArrayTranspose",
      },
      {
        text: "遍历树节点",
        link: "/page/frontend/JavaScript/3_ForeachTree",
      },
      {
        text: "数组转树结构|树形结构转数组",
        link: "/page/frontend/JavaScript/3_TreeDataTranslate",
      },
    ],
  },
  {
    text: "UtilsObject系列",
    collapsed: false,
    items: [
      {
        text: "对象深拷贝",
        link: "/page/frontend/JavaScript/4_ObjClone",
      },
      {
        text: "字符串转对象",
        link: "/page/frontend/JavaScript/4_StrParse",
      },
      {
        text: "删除无效属性",
        link: "/page/frontend/JavaScript/4_RemoveNullUndefined",
      },
      {
        text: "获取/反转对象键值对",
        link: "/page/frontend/JavaScript/4_InvertObj",
      },
      {
        text: "对象是否为空",
        link: "/page/frontend/JavaScript/4_IsEmptyObj",
      },
      {
        text: "(上传)对象转化为FormData对象",
        link: "/page/frontend/JavaScript/4_GetFormData",
      },
      {
        text: "检查对象是否存在某个属性",
        link: "/page/frontend/JavaScript/4_HasObjKey",
      },
    ],
  },
  {
    text: "UtilsNumber系列",
    collapsed: false,
    items: [
      {
        text: "中文金额相互转换数字",
        link: "/page/frontend/JavaScript/5_DigitUppercase",
      },
      {
        text: "使用'BigInt'支持大数计算",
        link: "/page/frontend/JavaScript/5_BigInt",
      },
      {
        text: "数字分隔符",
        link: "/page/frontend/JavaScript/5_ReadableBillion",
      },
      {
        text: "指定位数四舍五入",
        link: "/page/frontend/JavaScript/5_RoundNumber",
      },
      {
        text: "截断数字",
        link: "/page/frontend/JavaScript/5_ToFixedNumber",
      },
      {
        text: "数字补零0",
        link: "/page/frontend/JavaScript/5_FillZero",
      },
      {
        text: "判断为负数奇数偶数|整数相等",
        link: "/page/frontend/JavaScript/5_IsEvenMinus",
      },
      {
        text: "数字取整~~代替Math.floor()",
        link: "/page/frontend/JavaScript/5_FloatNumber",
      },
      {
        text: "数字千分位分隔|金额格式化",
        link: "/page/frontend/JavaScript/5_FormatMoney",
      },
    ],
  },
  {
    text: "UtilsDateTime系列",
    collapsed: false,
    items: [
      {
        text: "时间转换为刚刚几秒分时天月年",
        link: "/page/frontend/JavaScript/6_FormatPast",
      },
      {
        text: "计算距离下次生日还有多少天",
        link: "/page/frontend/JavaScript/6_GetBirthdayFun",
      },
      {
        text: "获取某年某月的第一天|最后一天",
        link: "/page/frontend/JavaScript/6_GetMonthDay",
      },
      {
        text: "获取某年某月的天数",
        link: "/page/frontend/JavaScript/6_GetMonthDays",
      },
      {
        text: "秒数转换",
        link: "/page/frontend/JavaScript/6_FormatSeconds",
      },
      {
        text: "判断日期是否为今天",
        link: "/page/frontend/JavaScript/6_IsToday",
      },
      {
        text: "查询某天是否为工作日",
        link: "/page/frontend/JavaScript/6_IsWeekday",
      },
      {
        text: "获取hh:mm:ss时间",
        link: "/page/frontend/JavaScript/6_TimeFormat",
      },
      {
        text: "查找日期位于一年中的第几天",
        link: "/page/frontend/JavaScript/6_DayOfYear",
      },
      {
        text: "检查日期是否有效",
        link: "/page/frontend/JavaScript/6_IsDateValid",
      },
      {
        text: "找出两日期之间的天数|日差",
        link: "/page/frontend/JavaScript/6_DayDif",
      },
    ],
  },
];
